...
# Model options
parser.add_argument('--input-dim', type=int, default=0, help="input dimension (default: 1024)")
...
parser.add_argument('--save-dir', type=str, default='log/image_rand_4_param_action16_imgto0_grad_clip_on_plocy', help="path to save output (default: 'log/')")
...
def main():
    ...
    for epoch in range(start_epoch, args.max_epoch):
        ...
        for ni, idx in enumerate(idxs):
            ...
            seq = torch.from_numpy(seq_feat) # input shape ( seq_len, dim) because they set batch first
            ...
            if use_gpu:
                ...
                # init_param = torch.cat([torch.tensor([[0.], [0.], [1.]]), torch.randn([1, 1]) ], dim=0).cuda()
                init_dist, _ = plane_metric(gt_param, init_param, torch.Tensor([0, 0, 0, 0]).cuda())
            ...
            total_loss = []
            epis_rewards = []
            for start_frm in range(0, seq.shape[0], args.num_episode):
                if  start_frm + args.num_episode >=seq.shape[0]:
                    if start_frm == 0:
                ...
                for nf in range(args.num_episode):
                    n_frm = start_frm + nf

                    dist, Qvalue  = model( param) # output shape (1, seq_len, 1) , seq[n_frm:n_frm+1,:],
                    action = dist.sample()

                    distance, fine_param = plane_metric(gt_param, param, action_space[action])
                    reward = step_reward + pre_dist - distance
                    if distance <= thresh:
                        if not b_arrive:
                            reward += 10
                            b_arrive = True
                    else:
                           reward += - 10 * step_reward
                ...
                #if distance > thresh:
                #    rewards[-1] -= 10 * distance / init_dist

                torch.cuda.synchronize()
                simu_time += time.time() - simu_time_

                _, next_value = model(fine_param) #seq[n_frm:n_frm+1,:],
                returns = compute_returns(next_value, rewards) # real reward we get

                log_probs = torch.cat(log_probs)
                returns = torch.cat(returns).detach()
                values = torch.cat(values)
                epis_rewards.append(torch.tensor([r.detach().item() for r in rewards]).sum())
            ...
            train_writer.add_scalar('epis_actor_loss', avg_actor_loss/cnt, epoch * len(train_keys) + ni)
            train_writer.add_scalar('epis_critic_loss', avg_critic_loss/cnt, epoch * len(train_keys) + ni)
            train_writer.add_scalar('epis_reward', np.mean(epis_rewards), epoch * len(train_keys) + ni)
            train_writer.add_scalar('epis_loss', loss, epoch * len(train_keys) + ni)
            ...
            # back_time += time.time() - back_time_
            if epoch % 100==0:
                train_writer.add_text('output_param_%s'%key, str(param), epoch )
        ...
        train_writer.add_scalar('epoch_loss', np.mean(total_loss), epoch)
