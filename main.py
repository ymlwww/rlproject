    ...
    for epoch in range(start_epoch, args.max_epoch):
        ...
        for ni, idx in enumerate(idxs):
            ...
            for _ in range(args.num_episode):
                ...
                for n_frm in range(seq.shape[0]):

                    dist, Qvalue  = model(seq[n_frm:n_frm+1,:], param) # output shape (1, seq_len, 1) ,
                    action = dist.sample()
                ...
                _, next_value = model(seq[n_frm:n_frm+1,:],fine_param) #seq[n_frm:n_frm+1,:],
            ...
            train_writer.add_scalar('epis_actor_loss', actor_loss/args.num_episode, epoch * len(train_keys) + ni)
            train_writer.add_scalar('epis_critic_loss', critic_loss/args.num_episode, epoch * len(train_keys) + ni)

            loss = actor_loss/args.num_episode + 0.5 * critic_loss/args.num_episode # - 0.001 * entropy
            total_loss += loss
            ...
            if ni % 10 == 0:
                print("epoch: {}: processing: {}/{}, avg_load_time: {}, simu_time: {}, back_time: {} ".format(
                    epoch, ni, len(idxs), loading_time/10, simu_time/10, back_time/10))
                simu_time = 0
                loading_time = 0
                back_time = 0
        ...
        train_writer.add_scalar('epoch_loss', total_loss/len(train_keys), epoch)
